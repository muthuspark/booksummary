...
Metcalf's "Marriage and Family Therapy" emphasizes a systemic approach. This means understanding individuals within their relational context—their family, their community, and their larger societal influences.  It shifts focus from blaming individuals for problems to examining the interactions and patterns within the system that contribute to difficulties.  The book argues that change in one part of the system inevitably affects the whole. This necessitates considering the interconnectedness of family members' behaviors, emotions, and roles.


## Circular Causality vs. Linear Causality

A core concept is the shift from linear to circular causality.  Linear causality suggests a simple cause-and-effect relationship (A causes B).  However, systemic therapy emphasizes circular causality, where actions and reactions create a continuous feedback loop.  For example, a child's misbehavior (A) might be a response to parental conflict (B), which is in turn exacerbated by the child's misbehavior (A).  Understanding this cyclical nature is crucial for effective intervention.
...
* **Narrative Therapy:**  Focuses on externalizing problems, separating the person from the problem, and constructing alternative narratives that emphasize strengths and resilience.
